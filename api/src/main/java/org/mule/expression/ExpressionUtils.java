...
public final class ExpressionUtils
{
    ...
    protected static <T> T getPropertyInternal(String expression, PropertyScope scope, boolean parseScope, MuleMessage msg, Class<T> type)
    {
        ...
        if (expression.contains(ExpressionConstants.ALL_ARGUMENT))
        {
            ...
            {
                //TODO i18n
                throw new IllegalArgumentException("Type specified is not a collection type but '" + ExpressionConstants.ALL_ARGUMENT + "' was specified for all properties. Type is: " + type);
            }
        }
        else if (Map.class.isAssignableFrom(type))
        {
            String[] names = expression.split(ExpressionConstants.DELIM);
            Map<String, Object> props = new HashMap<String, Object>();
            for (String name : names)
            {
                ...
                if (name.endsWith(ExpressionConstants.OPTIONAL_ARGUMENT))
                {
                    name = name.substring(0, name.length() - ExpressionConstants.OPTIONAL_ARGUMENT.length());
                    required = false;
                }
        ...
        {
            boolean required = true;
            if (expression.endsWith(ExpressionConstants.OPTIONAL_ARGUMENT))
            {
                expression = expression.substring(0, expression.length() - ExpressionConstants.OPTIONAL_ARGUMENT.length());
                required = false;
            }
